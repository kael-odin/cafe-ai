...
describe('AppManager', () => {
   ...
   * Uses the global __Cafe_TEST_DIR__ from setup.ts for filesystem paths.
   */
  function setup(): void {
  ...
  describe('install', () => {
    ...
    it('should create the work directory and memory subdirectory', async () => {
      ...
      const workDir = join(spacePaths[TEST_SPACE_ID], '.Cafe', 'apps', appId)
  ...
  describe('getAppWorkDir', () => {
    it('should return the work directory path', async () => {
      ...
      expect(workDir).toBe(join(spacePaths[TEST_SPACE_ID], '.Cafe', 'apps', appId))
      expect(existsSync(workDir)).toBe(true)
    })
