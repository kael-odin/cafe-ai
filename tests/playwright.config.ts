 ...
 *
 * Configures Playwright to test the Cafe Electron application.
...
function getAppPath(): string {
  ...
  if (platform === 'darwin') {
    ...
    const arm64Path = path.join(projectRoot, 'dist/mac-arm64/Cafe.app/Contents/MacOS/Cafe')
    const x64Path = path.join(projectRoot, 'dist/mac/Cafe.app/Contents/MacOS/Cafe')
    ...
    return path.join(projectRoot, 'dist/win-unpacked/Cafe.exe')
  } else {
    // Linux
    return path.join(projectRoot, 'dist/linux-unpacked/Cafe')
  }
}
