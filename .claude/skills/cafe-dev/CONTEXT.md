...
> Goal: quickly understand what Cafe does, its quality standards, and mandatory conventions.
...
Cafe = Claude Code SDK capabilities + ChatGPT-level UI/UX
...
Cafe is a local-first Electron AI product with:
...
- **Space management**: Cafe temporary space + dedicated spaces with centralized storage
...
- Configuration stored in `~/.cafe/config.json`; never hardcode secrets in source/docs.
