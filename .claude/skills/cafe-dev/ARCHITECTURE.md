...
├── temp/                       # Cafe temporary space (id: halo-temp)
│   ├── artifacts/
│   └── conversations/
...
- **`path`** (data path): Always centralized under `~/.cafe/spaces/{uuid}/`. Used for conversations, meta.json, and all persisted data.
