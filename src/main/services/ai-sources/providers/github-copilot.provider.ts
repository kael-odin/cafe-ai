...
// ============================================================================
// Persistent Identity
// Stored in ~/.cafe/config.json under copilot.identity, created once, never rotated.
 ...
 *   - Current cycle has been alive for more than idMaxAgeMinutes
 *
...
function getNextRequestIds(): {
  ...
  if (idRemainingUses <= 0 || now - idCycleStartedAt > idCurrentMaxAgeMs) {
    // Re-read config only on rotation — keeps the hot path I/O-free.
...
// ============================================================================
// Config Helpers (v1/v2 compatibility)
// ============================================================================
...
class GitHubCopilotProvider implements OAuthAISourceProvider {
  ...
  isConfigured(config: ProviderConfig): boolean {
    const c = this.getOAuthConfig(config)
