    ...
    if (scope === 'space' && spaceId) {
      ...
      if (spaceId === 'Cafe-temp') {
        ...
        try {
          ...
          if (space) {
            return this.scanConversationFiles(join(space.path, '.cafe', 'conversations'))
          }
        } catch (e) {
    ...
    if (scope === 'global') {
      ...
      if (existsSync(spacesDir)) {
        ...
        spaceNames.forEach(spaceName => {
          try {
            const convDir = join(spacesDir, spaceName, '.cafe', 'conversations')
  ...
  private findConversationFile(conversationId: string, spaceId?: string): string | null {
    ...
    if (spaceId) {
      if (spaceId === 'Cafe-temp') {
        ...
        try {
          const space = getSpace(spaceId)
          if (space) {
            const filePath = join(space.path, '.cafe', 'conversations', `${conversationId}.json`)
            if (existsSync(filePath)) {
              return filePath
            }
          }
    ...
    if (existsSync(spacesDir)) {
      ...
      for (const spaceName of spaceNames) {
        try {
          filePath = join(spacesDir, spaceName, '.cafe', 'conversations', `${conversationId}.json`)
          if (existsSync(filePath)) {
            return filePath
          }
        } catch (e) {
