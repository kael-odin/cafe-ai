/**
 * Cafe Apps Conversation MCP Server
 *
...
async function waitForAppManager(maxMs = 5000, intervalMs = 200) {
  ...
  console.log('[CafeAppsMcp] AppManager not ready, waiting...')
  let waited = 0
  while (waited < maxMs) {
    ...
    if (m) {
      console.log(`[CafeAppsMcp] AppManager ready after ${waited}ms`)
      return m
    }
  }
  console.error(`[CafeAppsMcp] AppManager still null after ${maxMs}ms — initPlatformAndApps may have failed`)
  return null
}
...
function buildTools(spaceId: string) {
  ...
  const create_automation_app = tool(
    ...
    '  - NEVER create config fields for cookies, session tokens, or any login credentials. The App runs inside the user\'s Cafe browser with shared session — authentication is automatic.\n\n' +
    ...
    async (args) => {
      try {
        ...
        if (!parsedSpec.author) parsedSpec.author = 'Cafe'
    ...
    async (args) => {
      try {
        ...
        try {
          ...
          console.warn(`[CafeAppsMcp] deactivate best-effort failed for ${args.app_id}:`, e)
        }
    ...
    async (args) => {
      try {
        ...
        try {
          ...
          console.warn(`[CafeAppsMcp] activate best-effort failed for ${args.app_id}:`, e)
        }
...
/**
 * Create Cafe Apps SDK MCP Server.
 ...
 */
export function createCafeAppsMcpServer(spaceId: string) {
