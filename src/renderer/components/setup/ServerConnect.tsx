/**
 * ServerConnect - Connection flow for adding a Cafe server (Capacitor mobile app).
 *
 * Allows the user to:
 * 1. Enter a Cafe server URL manually
...
export function ServerConnect({ onServerAdded, onBack }: ServerConnectProps) {
  ...
  const handleServerSubmit = async () => {
    ...
    try {
      ...
      setError(t('Cannot connect to server. Check the address and make sure Cafe is running.'))
    } finally {
  ...
  // Parse QR code result - expected format: cafe://<url>?code=<access_code>
  // or just a plain URL
        ...
        {step === 'scanning' && (
          <div className="w-full max-w-sm animate-fade-in">
            ...
            <p className="text-center text-sm text-muted-foreground mt-4">
              {t('Point your camera at the QR code on your desktop Cafe')}
            </p>
          </div>
        )}
          ...
          <div className="w-full max-w-sm space-y-4 animate-fade-in">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1.5">
                {t('Find this in Cafe desktop → Settings → Remote Access')}
              </p>
            </div>
        ...
        {step === 'auth' && (
          <div className="w-full max-w-sm space-y-4 animate-fade-in">
            ...
            <div>
              ...
              />
              <p className="text-xs text-muted-foreground mt-1.5">
                {t('Enter the 6-digit code shown on your desktop Cafe')}
              </p>
            </div>
